...
## :bell: 최종 결론 및 개선 방안
* OTT 산업 전, 후 에도 변화가 보이지만 코로나를 기점으로 OTT 산업과 영화 산업에서 큰 변화가 보인다.
* 장르 별로 추이에서 코로나 이후로 전체적으로 관객수가 줄었지만 흥행을 유지하거나 코로나 이전보다 매출이 증가한 장르들이 보인다.
* 특별 상영관의 관객수와 매출이 전부 증가한 것으로 보아 별한 경험과 같은 것을 기대한다는 것을 알 수 있다.

***

## 개선 방안 
1. TOP 50 순위를 뽑았다고 하나 영화의 개수로만 판단하다 보니 실질적으로 흥행을 한 결과와 달라질 가능성이 있다. 예를 들어 1, 2, 3 위가 액션이고 그 아래에 뮤지컬 영화가 5개가 있다면 액션 영화가 뮤지컬 영화보다 흥행했다고 판단이 되어야 하는데 현재 분석 결과로는 뮤지컬 영화가 더 흥행했다고 판단하게 된다. 이 점을 개선하기 위해 이후에 장르 개수 분석을 할 때 영화 순위 별 가중치를 주어 분석을 하는 방향으로 가면 좋을 것 같다.
2. 장르 분석에서 대충 짐작을 하고 넘어갔던 관객수와 매출, 상영 횟수의 경향이 다른 경우의 정확한 원인이 분석되지 않았다. 이 점을 개선하기 위해 이후에 더 추가적으로 상영관 별 가격과 관객수 변화 등을 분석해 보면 좋을 것 같다.
3. 유튜브의 영화 요약 정리 영상 등 영화 산업에 영향을 미치는 다른 요소들도 존재여여 영화 산업이 OTT로 인해서만 변화한 것이 아니기에 추이가 정확하게 제대로 분석이 되지는 않았다. 이를 개선하기 위해 이후에 추가적으로 사람들의 문화 산업 소비 행태에 대하여 영화가 어떻게 변화하는지 분석해 보면 좋을 것 같다. 
